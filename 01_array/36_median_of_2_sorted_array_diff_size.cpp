...
int getMedian(int ar1[], int ar2[], int n, int m)
{
    ...
    if ((m + n) % 2 == 1)
    {
        ...
        {
            if (i != n && j != m)// dono hi h to compare
            {
                m1 = (ar1[i] > ar2[j]) ? ar2[j++] : ar1[i++];
            }
            else if (i < n)// bacha hua arr1 ki values
            {
                m1 = ar1[i++];
            }
            // for case when j<m,
    ...
    else
    {
        for (count = 0; count <= (n + m) / 2; count++)
        {
            ...
            if (i != n && j != m)
            {
                m1 = (ar1[i] > ar2[j]) ? ar2[j++] : ar1[i++];         // m1=3(age vla) m2=3 (prev)
            }
            else if (i < n)
            {
                m1 = ar1[i++];
            }
            // for case when j<m,                        1 2 3          3 4 5 7
            else                            // 1 2 3 3 4 5 7
            {
                m1 = ar2[j++];
            }
        }
        return (m1 + m2) / 2;      // 3+2/2=2
    }
}
/* Driver code */
int main()
